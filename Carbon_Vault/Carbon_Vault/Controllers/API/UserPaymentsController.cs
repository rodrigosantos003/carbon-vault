...
namespace Carbon_Vault.Controllers.API
{
    ...
    public class UserPaymentsController : ControllerBase
    {
        ...
        private async Task AddPurchaseTransactionAsync(int userId, int projectId, int quantity, string paymentMethod, string checkoutSession)
        {
...
=======
        ...
        public async Task<IActionResult> GetSessionDetails(string sessionId)
        {
            try
            {
                ...
                var result = new
                {
                    AmountTotal = session.AmountTotal / 100.0, // Convert from cents to currency
                ...
                await AddPurchaseTransactionAsync(result.UserId, result.FirstItemId, result.FirstItemQuantity, result.PaymentMethod, sessionId);
